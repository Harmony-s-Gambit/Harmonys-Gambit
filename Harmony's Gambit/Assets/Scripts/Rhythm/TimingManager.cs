...
public class TimingManager : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        {
            _timingBoxsP1[i].Set(_centerP1.localPosition.y - _timingRectP1[i].rect.height / 2, _centerP1.localPosition.y + _timingRectP1[i].rect.height / 2); //각 판정의 y좌표 범위 설정, 정확한 판정일수록 범위 작음
        }

        _timingBoxsP2 = new Vector2[_timingRectP2.Length];
        for (int i = 0; i < _timingRectP2.Length; i++)
        {
            _timingBoxsP2[i].Set(_centerP2.localPosition.y - _timingRectP2[i].rect.height / 2, _centerP2.localPosition.y + _timingRectP2[i].rect.height / 2);
        }
    }
    ...
    public void CheckTiming(int playerNum, string key) //키를 눌렀을 때 실행
    {
        if (playerNum == 1) //플레이어 1이라면
        {
            ...
            {
                float t_notePosY = boxNoteListP1[i].transform.localPosition.y; //노트의 y좌표 받아오기
                for (int j = 0; j < _timingBoxsP1.Length; j++) //판정 범위만큼 실행, 현재 1개이므로 한 번만 실행하게 됨
                {
                    if (_timingBoxsP1[j].x <= t_notePosY && t_notePosY <= _timingBoxsP1[j].y) //판정 범위 안에 있는가
                    {
        ...
        else
        {
            ...
            {
                float t_notePosY = boxNoteListP2[i].transform.localPosition.y;
                for (int j = 0; j < _timingBoxsP2.Length; j++)
                {
                    if (_timingBoxsP2[j].x <= t_notePosY && t_notePosY <= _timingBoxsP2[j].y)
                    {
    ...
    private void RecordNoteYPos(int player, float yPos) //오프셋 설정 시 필요함, 현재 노트의 x좌표를 넘겨줌
    {
        if (NoteManager.instance.currentBGM == "Offset")
        {
            GenerateNoteTest.instance.RecordingXPos(player, yPos);
        }
    }
}
